// ─── Egyptian Emergency Numbers per category ───────────────
const egyptNumbers = {
  Cardiac:     [{ num: "123", label: "Ambulance" }, { num: "122", label: "Police" }],
  Choking:     [{ num: "123", label: "Ambulance" }],
  Bleeding:    [{ num: "123", label: "Ambulance" }, { num: "122", label: "Police" }],
  Burns:       [{ num: "123", label: "Ambulance" }, { num: "137", label: "Fire Dept" }],
  Fractures:   [{ num: "123", label: "Ambulance" }, { num: "122", label: "Police" }],
  Poisoning:   [{ num: "123", label: "Ambulance" }, { num: "180", label: "Poison Control" }],
  Seizures:    [{ num: "123", label: "Ambulance" }],
  Heat:        [{ num: "123", label: "Ambulance" }],
  Hypothermia: [{ num: "123", label: "Ambulance" }],
  Drowning:    [{ num: "123", label: "Ambulance" }, { num: "122", label: "Police" }],
  Shock:       [{ num: "123", label: "Ambulance" }, { num: "122", label: "Police" }],
};
...
function renderEmergencies() {
  ...
  if (filtered.length === 0) {
    ...
    grid.innerHTML = filtered.map(emergency => {
      ...
      const nums = egyptNumbers[emergency.category] || [{ num: "123", label: "Ambulance" }];
      const numsHTML = nums
        .map(n => `<a href="tel:${n.num}" class="egy-badge" onclick="event.stopPropagation()">
            <i class="fa-solid fa-phone"></i>
            <span class="egy-num">${n.num}</span>
            <span class="egy-lbl">${n.label}</span>
          </a>`)
        .join('');
      return `
        <div class="emergency-card" onclick="showEmergencyAlert(${emergency.id})">
          ...
          <div class="emergency-card-body">
            ...
            <div class="egy-numbers-box">
              <span class="egy-title"><i class="fa-solid fa-phone-volume"></i> Egypt Emergency Numbers</span>
              <div class="egy-list">${numsHTML}</div>
            </div>
          </div>
...
function showEmergencyAlert(emergencyId) {
  ...
  // Egyptian numbers inside modal
  const modalNums = egyptNumbers[emergency.category] || [{ num: "123", label: "Ambulance" }];
  const modalNumsHTML = modalNums
    .map(n => `<a href="tel:${n.num}" class="egy-badge modal-egy-badge">
        <i class="fa-solid fa-phone"></i>
        <span class="egy-num">${n.num}</span>
        <span class="egy-lbl">${n.label}</span>
      </a>`)
    .join('');
  // Insert/update the numbers block right inside the modal body (before immediate section)
  const alertBody = document.getElementById('emergency-alert-body');
  let existingNums = alertBody.querySelector('.modal-egy-block');
  if (existingNums) existingNums.remove();
  const numsBlock = document.createElement('div');
  numsBlock.className = 'modal-egy-block';
  numsBlock.innerHTML = `
    <div class="modal-egy-label"><i class="fa-solid fa-phone-volume"></i> Egypt Emergency Numbers</div>
    <div class="egy-list">${modalNumsHTML}</div>`;
  alertBody.insertBefore(numsBlock, alertBody.firstChild);
...
// Initial render
renderEmergencies();
// ─── Inject Egyptian numbers styles ───────────────────────
(function () {
  ...
  s.textContent = `
    ...
    .egy-numbers-box {
      margin: 14px 0 4px;
      background: linear-gradient(135deg,rgba(200,16,46,.08),rgba(200,16,46,.03));
    ...
    .egy-title i { margin-right: 5px; }
    .egy-list {
      display: flex;
      flex-wrap: wrap;
      gap: 7px;
    }
    /* ── Badge (shared card + modal) ── */
    .egy-badge {
      display: inline-flex;
      align-items: center;
      gap: 6px;
    ...
    .egy-badge i { font-size: .73rem; opacity: .85; }
    .egy-num { font-size: 1rem; letter-spacing: .03em; }
    .egy-lbl { font-size: .7rem; font-weight: 500; opacity: .88; }
    /* ── Modal numbers block ── */
    .modal-egy-block {
      background: linear-gradient(135deg,rgba(200,16,46,.1),rgba(200,16,46,.04));
    ...
    .modal-egy-label i { margin-right: 6px; }
    .modal-egy-badge {
      padding: 7px 16px 7px 11px;
      font-size: .88rem;
    }
    .modal-egy-badge .egy-num { font-size: 1.08rem; }
    .modal-egy-badge .egy-lbl { font-size: .73rem; }
  `;
...
// Wait for widget to load then hook the toggle button
var currentLang = "en";
var toggleBtn = document.getElementById("lang-toggle");

if (toggleBtn) {
  toggleBtn.addEventListener("click", function () {
    // Try to use Google Translate select if available
    var select = document.querySelector(".goog-te-combo");
    if (!select) {
      // Widget not ready yet — retry once after 1.5s
      setTimeout(function () {
        toggleBtn.click();
      }, 1500);
      return;
